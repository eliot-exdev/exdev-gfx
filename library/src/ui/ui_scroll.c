...
void ui_scroll_container_update(UIScrollContainer_t *self, const long time_elapsed, const Event_t *events, const int num_events, UIApplication_t *app, void *usr_ptr) {
    assert(self);
    assert(events);
    ...
    if (self->h_bar) {
        self->h_bar->base.functions.update_func(self->h_bar, time_elapsed, events, num_events, app, usr_ptr);
    }
